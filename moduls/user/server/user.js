...
if (Meteor.settings.mail) {
	...
	Accounts.emailTemplates.verifyEmail.text = function(user, url) {
		var text = 'Нажав на эту ссылку, вы подтверждаете, что данная почта недоступна Рептилоидам, что канал связи с параллельной вселенной защищён и вы, истинно Великий Консул - являетесь единственным пользователем данного аккаунта. Удачи в Бою и да прибудет с вами Летающий Макаронный Монстр.: ' + url;
		text += '\n\n' + 'Этот адрес электронной почты был получен нами после вашей регистрации в игре ConsulWar и подключения к каналу связи. Это письмо вы получили, так как был запущен новый сервер и открыт новый канал связи.';
		text += '\n\n' + 'Если вас больше не интересует спасение параллельной вселенной, и вы больше не хотите получать рассылку от проекта ConsulWar, нажмите на ссылку: ';
		text += 'http://consulwar.ru/unsubscribe@mail=' + user.emails[0].address;
		text += '\n\n' + 'После этого вы перестанете получать письма от проекта ConsulWar';
		return text;
	};
